...
test("extracts asterisk crossover pair intent", () => {
  const resolution = resolveQueryNodes("Dune * Roman Empire");
  assert.deepEqual(pairIds("Dune * Roman Empire"), ["dune", "roman-empire"]);
  assert.equal(resolution.strategy, "explicit_pair");
  assert.equal(resolution.needsClarification, false);
...
test("extracts pair intent with trailing punctuation in tokens", () => {
  const resolution = resolveQueryNodes('Connect "Dune," to "Roman Empire..."');
  assert.deepEqual(pairIds('Connect "Dune," to "Roman Empire..."'), ["dune", "roman-empire"]);
  assert.equal(resolution.strategy, "explicit_pair");
  assert.equal(resolution.needsClarification, false);
...
test("extracts pair intent with leading punctuation in tokens", () => {
  const resolution = resolveQueryNodes('Connect ":Dune" to "...Roman Empire"');
  assert.deepEqual(pairIds('Connect ":Dune" to "...Roman Empire"'), ["dune", "roman-empire"]);
  assert.equal(resolution.strategy, "explicit_pair");
  assert.equal(resolution.needsClarification, false);
...
test("extracts pair intent with unicode punctuation in tokens", () => {
  const resolution = resolveQueryNodes('Connect "…Dune" to "Roman Empire…"');
  assert.deepEqual(pairIds('Connect "…Dune" to "Roman Empire…"'), ["dune", "roman-empire"]);
  assert.equal(resolution.strategy, "explicit_pair");
  assert.equal(resolution.needsClarification, false);
...
test("extracts colon-style crossover pair intent", () => {
  const resolution = resolveQueryNodes("Dune : Roman Empire");
  assert.deepEqual(pairIds("Dune : Roman Empire"), ["dune", "roman-empire"]);
  assert.equal(resolution.strategy, "explicit_pair");
  assert.equal(resolution.needsClarification, false);
...
test("extracts full-width colon crossover pair intent", () => {
  const resolution = resolveQueryNodes("듄 ： 로마 제국");
  assert.deepEqual(pairIds("듄 ： 로마 제국"), ["dune", "roman-empire"]);
  assert.equal(resolution.strategy, "explicit_pair");
  assert.equal(resolution.needsClarification, false);
...
test("extracts middle-dot crossover pair intent", () => {
  const resolution = resolveQueryNodes("Dune · Roman Empire");
  assert.deepEqual(pairIds("Dune · Roman Empire"), ["dune", "roman-empire"]);
  assert.equal(resolution.strategy, "explicit_pair");
  assert.equal(resolution.needsClarification, false);
...
test("extracts japanese middle-dot crossover pair intent", () => {
  const resolution = resolveQueryNodes("듄 ・ 로마 제국");
  assert.deepEqual(pairIds("듄 ・ 로마 제국"), ["dune", "roman-empire"]);
  assert.equal(resolution.strategy, "explicit_pair");
  assert.equal(resolution.needsClarification, false);
...
test("extracts equals-sign crossover pair intent", () => {
  const resolution = resolveQueryNodes("Dune = Roman Empire");
  assert.deepEqual(pairIds("Dune = Roman Empire"), ["dune", "roman-empire"]);
  assert.equal(resolution.strategy, "explicit_pair");
  assert.equal(resolution.needsClarification, false);
...
test("extracts crossed-swords emoji crossover pair intent", () => {
  const resolution = resolveQueryNodes("Dune ⚔️ Roman Empire");
  assert.deepEqual(pairIds("Dune ⚔️ Roman Empire"), ["dune", "roman-empire"]);
  assert.equal(resolution.strategy, "explicit_pair");
  assert.equal(resolution.needsClarification, false);
...
test("extracts right-arrow crossover pair intent", () => {
  const resolution = resolveQueryNodes("Dune → Roman Empire");
  assert.deepEqual(pairIds("Dune → Roman Empire"), ["dune", "roman-empire"]);
  assert.equal(resolution.strategy, "explicit_pair");
  assert.equal(resolution.needsClarification, false);
...
test("extracts double-arrow crossover pair intent", () => {
  const resolution = resolveQueryNodes("듄 ⇒ 로마 제국");
  assert.deepEqual(pairIds("듄 ⇒ 로마 제국"), ["dune", "roman-empire"]);
  assert.equal(resolution.strategy, "explicit_pair");
  assert.equal(resolution.needsClarification, false);
